...
class FFNN(object):
    ...
    def build(self):
        ...
        self.input = tf.placeholder("float", [None, self.nvars])
        W1 = tf.Variable(tf.random_uniform([self.nvars,20], -.01, .01))
        W2 = tf.Variable(tf.random_uniform([20,1], -.01, .01))

        hidden1 = tf.nn.relu(tf.matmul(self.input, W1) + b1)
        self.y = tf.sigmoid(tf.matmul(hidden1, W2) + b2)

        self.ideal = tf.placeholder("float", [None,1])

        self.loss = tf.reduce_sum(tf.pow((self.ideal-self.y), 2))
